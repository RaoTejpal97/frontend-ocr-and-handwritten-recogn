...
const onFormSubmit = (event) => {
    ...
    const output_div_start = '<div  class="flex justify-center py-10 mt-1"><div class="flex justify-center py-5 mt-1 rounded-lg shadow-xl bg-gray-50 lg:w-1/2 flex p-5 space-x-4 my-4 min-w-max w-max">'
    // const output_div_start = '<div><div>'
    const output_div_end = '</div></div>'
            
    const result_div = document.getElementById("result")

    result_div.innerHTML = output_div_start + '<h1 id="predicted_text">Loading...</h1>' + output_div_end

    const predicted_text_html = document.getElementById("result")
    ...
    axios
        ...
        .then((res) => {
            ...
            if(input_type==='Handwriting Recognition'){
                result_div.innerHTML = output_div_start + '<h1 id="predicted_text">'+predicted_text+ '</h1>' + output_div_end
            }else{
                predicted_text_parsed = JSON.parse(predicted_text)
                var text_to_be_displayed_on_html_page = '<p id="predicted_text">'
                var counter = 0
                for(let key in predicted_text_parsed){
                    text_to_be_displayed_on_html_page += key+": "+predicted_text_parsed[key]+"<br>"
                }
                text_to_be_displayed_on_html_page += '</p>'
                result_div.innerHTML = output_div_start + '<div>'+text_to_be_displayed_on_html_page+'</div>'+output_div_end
                console.log(text_to_be_displayed_on_html_page)
                predicted_text_html.style.color = 'green'
                predicted_text_html.style['font-size'] = '28px'
                predicted_text_html.style.fontWeight = '600'
            }
